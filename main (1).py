...
try:
    pygame.mixer.init()
    pygame.mixer.music.set_volume(0.8)  # Set a moderate volume level
...
logo_frame.pack(pady=10)  # Add some padding

# Load and display the logo
try:
    logo_image = Image.open("Project Beatviz.png")  # Replace with the path to your PNG logo
...
def update_visualizer():
    ...
    if is_playing and current_data is not None:
        playback_position = pygame.mixer.music.get_pos() / 1000  # Convert ms to seconds
        samples_to_display = int(playback_position * current_sr)

        # Sliding window for the waveform
...
def load_audio_file():
    ...
    if file_path:
        ...
        try:
            ...
            playback.play(audio)  # Alternative method if pygame playback fails

def play_example_track():
    """Play an example audio track and display the oscillogram."""
    file_path = "R&B Example.wav"  # Replace with your example track path
    audio, sr = process_audio(file_path)
    plot_oscillogram(audio, sr)
